...
import code.ticketreservationapp.service.EmailJsNotifier;
...
public class UserPage extends BasePageActivity {
    ...
    protected void onStart() {
        super.onStart();
        if (!ensureAuthorized(LoginRole.USER)) {
            return;
        }
        refreshAll();
    }
    ...
    private void setupListeners() {
        ...
        findViewById(R.id.switchToAdminButton).setOnClickListener(view -> logoutAndReturnToWelcomePage());
    ...
    private void handleReserve(Event event, TextInputEditText quantityInput) {
        try {
            ...
            refreshAll();
            promptConfirmationDelivery(reservation);
        } catch (IllegalArgumentException ex) {
            showMessage(ex.getMessage());
        }
    }

    private void promptConfirmationDelivery(Reservation reservation) {
        String[] options = {"Email", "SMS (mock)"};
        new AlertDialog.Builder(this)
                .setTitle("Send confirmation via")
                .setItems(options, (dialog, which) -> {
                    if (which == 0) {
                        sendEmailConfirmation(reservation);
                        return;
                    }
                    sendMockSmsConfirmation(reservation);
                })
    ...
    private void sendEmailConfirmation(Reservation reservation) {
        ...
        EmailJsNotifier.sendReservationConfirmation(email, reservation, (success, message) ->
                runOnUiThread(() -> showMessage(message))
        );
    }

    private void sendMockSmsConfirmation(Reservation reservation) {
        String phone = AuthSessionStore.currentPhone();
        String destination = TextUtils.isEmpty(phone) ? "phone on file" : phone;
    ...
    private String resolveConfirmationEmail(Reservation reservation) {
        ...
        if (registeredUser != null
                && registeredUser.getContactMethod() == ContactMethod.EMAIL
                && isValidEmail(registeredUser.getContactValue())) {
            return registeredUser.getContactValue().trim();
        }

        if (reservation.getChannel() == ContactMethod.EMAIL && isValidEmail(reservation.getContactValue())) {
            return reservation.getContactValue().trim();
        }
